#! /usr/bin/env python
#"Time-stamp: <03/28/2015  2:08:56 AM rsg>"
__doc__ = '''
...
if __name__ == "__main__":
    ...
    if myOptions.command == 'getstatusoutput':
        ...
        tsSysCommands.TsSysCommands.tsGetStatusOutput(myArgs[0])

    elif myOptions.command == 'getoutput':

        tsSysCommands.TsSysCommands.tsGetOutput(myArgs[0])

    elif myOptions.command == 'getstatus':

        tsSysCommands.TsSysCommands.tsGetStatus(myArgs[0])

    command = 'stty size' #'ping 10.0.160.100'
    print(command)

    tsSysCommands.TsSysCommands.tsCheckCall(command.split(' '))
