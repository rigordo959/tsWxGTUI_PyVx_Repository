#! /usr/bin/env python
#"Time-stamp: <04/22/2015  7:28:07 AM rsg>"
'''
tsLOCPMOperatorSettingsParser.py - Class of utility functions,
used by the tsLinesOfCodeProjectMetrics.py tool,
to parse the command line entered by the operator of an
application program. Parsing extracts the Keyword-Value
pair Options and Positional Arguments that will configure
and control the application during its execution.
'''
#################################################################
#
# File: tsLOCPMOperatorSettingsParser.py
#
# Purpose:
#
#     Class of utility functions to parse the command line
#     entered by the operator of an application program.
...
# Usage (example):
#
#     import tsLOCPMOperatorSettingsParser
#
# Methods:
#
#    __init__ - TsLOCPMOperatorSettingsParser Class constructor.
#
...
__title__     = 'tsLOCPMOperatorSettingsParser'
__version__   = '2.9.0'
__date__      = '03/23/2015'
...
evolving Python 2.x releases (currently 2.7.9). The application
...
Python 3.x release (currently 3.4.3) compatible version
...
The software source code metrics algorithm scans a directory
tree of source code input files and produces output to a
statistics file and to the CLI display that reports:

    File-by-File Details for recognized file name extensions:

        file path, name and extension
        programming language type (via its filename extension),
        number of lines of code
        number of blank/comment lines
        number of words
        number of characters

    File-by-File Details for unrecognized file name extension:

        file path, name and extension

    Summay:

        accumulated number of lines of code
        accumulated number of blank/comment lines
        accumulated number of words
        accumulated number of characters
        standard deviation in number of lines of code
        average number of lines of code
        relative percentage of code and comment lines.

The software project metrics algorithm is based on the
Constructive Cost Model (COCOMO), an algorithmic software
cost estimation model developed by Dr. Barry W. Boehm, first
published in his 1981 book "Software Engineering Economics".
The model uses a basic regression formula with parameters
that are derived from historical project data and current
as well as future project characteristics.

    COCOMO models ("organic" is default):

    "organic" - "small" teams with "good" experience working
    with "less than rigid" requirements

    "semi-detatched" - "medium" teams with mixed experience
...
Unless invoked by a command line option that selects another
of the three COCOMO models the software project metrics
algorithm uses the default model to analyze the software source
code metrics output and generate additional output to the
statistics file and to the CLI display that reports:

    Total Physical Source Lines of Code (KSLOC)

    Estimated Development Effort in Person-Years (Person-Months)
...
gather software source file metrics (source type, code lines,
comment lines, total lines, words, characters) for each invidual
file and for collection of files in this input directory
(default = "./").
...
log/display source file metrics (source type, code lines, comment
lines, total lines, words, characters) for each invidual file and
for collection of  files to this output file
(default = "./%progStatistics.txt")
...
projectHelp = textwrap.dedent('''

log/display software development project metrics in accordance with
COCOMO Model, first published, in 1981, by Dr. Barry W. Boehm.
Key choice: {"0" (default) | "1"  | "2"}

    where:

        "0" selects "organic" model

              "small" teams with "good" experience
              working with "less than rigid"
              requirements;

        "1" selects "semi-detatched" model

              "medium" teams with mixed experience
              working with a mix of "rigid" and
              "less than rigid" requirements;

         "2" selects "embedded" model

              developed within a set of "tight"
              constraints. It is also combination
              of "organic" and "semi-detached"
...
class TsLOCPMOperatorSettingsParser(object):
    '''
...
##            break_long_words=True)
        ...
        self.options = {
            ...
            'input':   './',
            'output':  './%sStatistics.txt' % self.getRunTimeTitle(),
            'project': '0',
            'scan':    False,
            'Verbose': False,
...
##                sys.exit(0)
            ...
            elif keyword in ('-p', '--project'):

                self.Project = value
                resultsOptions['project'] = value
    ...
    def getProject(self):
        '''
        Return default value, after initialization and input value after
        operator entered "-p"/"--project" key-word option.
        '''
    ...
    def setProject(self, value):
        '''
        Set value after operator entered "-p"/"--project" key-word option.
        '''
        self.options['project'] = value
...
##        return (theSetProperties)
    ...
    def helpAbout(self):
        ...
        try:
            ...
            sampleOutput = './tsToolsCLI/tsLinesOfCodeProjectMetricsPkg' + \
                           '/sampleHelp' + \
                           '/%sstatistics.txt' % self.getRunTimeTitle()
            ...
            fmt2 = '.tsLOCPMOperatorSettingsParser: ' + \
                   'errorCode="%s"\n' % str(errorCode)
            msg = fmt1 + fmt2
    ...
    def helpSoftwareMetrics(self):
        '''
        helpSoftwareMetrics
        '''
        if EnableEarliestAncestor:
            print(self.helpSoftwareMetrics.__doc__)
        ...
        if len(rawArgsOptions) >= 0:
            ...
            for arg in rawArgsOptions:
                ...
                if arg.lower() == 'argparse':
                    ...
                    theParserModule = theParserModule = 'getopt'
                    break
...
##            print('\t\tProject=%s' % str(self.Project))
...
##        import argparse
        ...
        parser.add_argument(
            ...
            action='store',
            dest='output',
            default='./%sStatistics.txt' % self.getRunTimeTitle(),
            type=str,
        ...
        parser.add_argument(
            ...
            dest='project',
            choices=['0', '1', '2'],
            help=textwrap.dedent(projectHelp).replace(
                '%prog', '%s.py' % self.getRunTimeTitle()))
...
##        from optparse import OptionParser
        ...
        parser.add_option(
            ...
            dest='project',
            choices=['0', '1', '2'],
            help=self.dedentFormatter(projectHelp))
...
##        options['help'] = optionsOptParse.help
        ...
        options['project'] =  optionsOptParse.project
