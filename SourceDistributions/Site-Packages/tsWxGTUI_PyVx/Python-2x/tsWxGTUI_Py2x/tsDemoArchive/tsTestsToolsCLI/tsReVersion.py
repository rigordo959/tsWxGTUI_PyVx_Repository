#! /usr/bin/env python
#"Time-stamp: <06/23/2009  3:21:23 AM rsg>"
'''
tsReVersion.py - Tool to generate a directory of publishable source
files. It transforms copyright and license macros into the latest
version of the copyright and license notices.
'''
#################################################################
#
# File: tsReVersion.py
...
# Purpose:
#
#     Tool to generate a directory of publishable source files.
#     It transforms copyright and license macros into the latest
#     version of the copyright and license notices.
#
# Limitations:
#
...
# Usage (examples):
#
#     python tsReVersion -d sourcePath -o targetPath -n noticePath
#
# ToDo:
#   1. TBD
#
...
# Modifications:
#
#   09/04/2007 rsg Created output directory when appropriate.
#
#################################################################
#@TeamSTARS.Copyright@
#@TeamSTARS.Proprietary@
#################################################################
import os
...
__line1__ = '%s, v%s (%s)' % (__title__, __version__, __date__)
...
if __name__ == '__main__':
    ...
    def replaceMacros(inputName, outputName, noticeDirectory):
        '''
        Replaces all instances of the copyright and proprietary rights
        macros with the contents of their associated expansion files.
        '''
        oldVersion = 'wxPython 2.8.8.1'
        newVersion = 'wxPython 2.8.9.2'
        oldTypo = 'Limitationsa:'
        newTypo = 'Limitations:'
        try:
            ...
            ##    print line
                if str(line).find(oldVersion) > -1:
                    ...
                    outputFile.write('%s' % line.replace(oldVersion,
                                                         newVersion))
                elif str(line).find(oldTypo) > -1:
                    changed = True
                    outputFile.write('%s' % line.replace(oldTypo,
                                                         newTypo))
                else:
    ...
    def getOptions():
        ...
        Generates a directory of publishable source files.
        It transforms copyright and license macros into the latest
        version of the copyright and license notices.
        '''
        )
