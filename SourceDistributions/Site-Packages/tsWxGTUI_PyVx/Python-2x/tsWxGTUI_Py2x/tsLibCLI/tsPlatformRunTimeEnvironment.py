#! /usr/bin/env python
#"Time-stamp: <04/20/2015  7:53:05 AM rsg>"
'''
...
#
#    2015/04/20 rsg Added import of pwd module.
#
# ToDo:
#
...
from tsWxGTUI_Py2x.tsLibCLI import tsGistGetTerminalSize
...
class PlatformRunTimeEnvironment(object):
    ...
    def tsGetHostConsoleDisplaySize(self, theInfo):
        ...
        try:
            ...
            sizex, sizey = tsGistGetTerminalSize.get_terminal_size()
...
##        if self.theSystem != 'Windows':
        try:
            ...
            try:
                ...
                try:
                    # pylint reports self.logger undefined error
                    self.logger.warning(
                        'tsPlatformRunTimeEnvironment.' + \
                        'tsGetProcessParameters: errorCode=%s' % str(
                            errorCode))
                except:
                    # work around for pylint error
                    timestamp = tsrpu.getDateTimeString(
                        time.time(), msec=True)
                    theCategory = "WARNING"
                    msg = 'tsPlatformRunTimeEnvironment.' + \
                          'tsGetProcessParameters: errorCode=%s' % str(
                              errorCode)
...
if __name__ == '__main__':
    ...
    except tse.InputOutputException, e:
        msg = str(e)
        exitStatus = e.exitCode
 
    except tse.ProgramException, e:
        msg = str(e)
        exitStatus = e.exitCode
