#-----------------------------------------------------------
#"Time-stamp: <01/16/2017  7:53:05 AM rsg>
#-----------------------------------------------------------
...
===================== File: README.txt =====================
   ...
   * 32-bit processors, nCurses 6.x/5.x, 32-bit Python 3.5.2
   ...
   Toolkit subdirectory named:
       ...
       (e.g. "tsWxGTUI_PyVx_SWIG_Repository") 
         |
         |
         +-- ["Documents"]
...
============================= Preface ===========================
   ...
   The TeamSTARS "tsWxGTUI_PyVx_SWIG" Toolkit's cross-platform
   Virtual Machine design and implementation supports a broad
   assortment of open and proprietary hardware and software
   platforms.

   Whereas the Python Standard Curses library only provides a
   minimal subset of the traditional BSD and AT&T C/C++ language
   based "Curses" Application Programming Interface (API), when
   the need justifies the effort, the Software Interface Generator
   (SWIG) development tool (from "http://swig.org/index.php") can
   be used to create alternative, full, customized Python versions
   of the "Curses" API for the following:

   * Ken Arnold's (termcap based 1st generation)

          BSD Unix "Curses" (32-bit)

             video highlighting (standout/reverse video)

             line-drawing (hyphen "-" for horizontal line,
             "|" for vertical line and plus sign "+" for
             line intersection)

             colors (single screen phosphor color, such as
             "white", "orange" or "green" which becomes
             visible when turned "ON" by the Cathode Ray
             Tube (CRT) electron beam and which becomes
             invisible when turned "OFF" by the absence
             of the CRT electron beam.

   * Mark Horton's (terminfo based 2nd generation)

          AT&T Unix "Curses" (32-bit)

             video highlighting (bold, underline, standout)

             line-drawing (horizontal, vertical and intersection)

             colors (three screen phosphor colors, such as "red",
             "green" and "blue" which become visible when turned
             "ON" by the three Cathode Ray Tube (CRT) electron
             beams. The relative intensities of the three CRT
             electron beams determines the number of colors
             (such as 8 or 16) in the displayed foreground and
             background color palette with the number of color
             pairs (64 and 256).

   * William McBrine's (an implementation of the
     "Curses" library for environments that don't fit
     the termcap/terminfo model [Win32, DOS, OS/2, X11 and SDL])

          Public Domain "PDCurses" 3.4.x (32-bit)

   * Thomas E. Dickey's (for The Free Softwre Foundation)
     a free software emulation of "Curses" in AT&T System V
     Release 4.0 (SVr4), and more. It uses terminfo format,
     supports pads and color and multiple highlights and
     forms characters and function-key mapping, and has
     all the other SVr4-curses enhancements over BSD curses.
     SVr4 curses is better known today as X/Open Curses.

     It supports terminals and terminal emulators having a
     single color phosphor (such as the Digital Equipment
     Corporation VT100) and those with three color phosphors.

     Platforms with 32-bit processors (or 64-bit ones operating
     in 32-bit compatibility mode) support up to 16 colors:

          GNU "nCurses"  1.x-5.x (32-bit)
          GNU "nCurses"  6.x (32-bit)

     Platforms with 64-bit processors support up to 256 colors:

          GNU "nCurses"  6.x (64-bit)

========================== Introduction =========================

1. Introduction

   The TeamSTARS "tsWxGTUI_PyVx_SWIG" Toolkit is released
   as Free and Open Source Software. You can get all of the
   source code and associated building-blocks, tools, tests,
   utilities, examples and documentation via the "GitHub"
   Web-based repository hosting service.

   Implemented in both the mature Python 2x and evolving
   Python 3x programming languages, its source code is that
   ...
   1.3 Toolkit Components
       ...
       1.3.1 "Site-Packages"
             ...
             The TeamSTARS "tsWxGTUI_PyVx_SWIG" Toolkit repository
   ...
   1.5 What should you do to get started?
       ...
       1.5.1 What is the TeamSTARS "tsWxGTUI_PyVx_SWIG" Toolkit?
...
2. Release Distribution Nominclature
   ...
   Multiple Python version-specific TeamSTARS "tsWxGTUI_PyVx_SWIG"
   ...
   2.1 <Release Name>:
       ...
       "tsWxGTUI_PyVx_SWIG"   - Designates the entire collection of
...
3. Release Distribution File Type
   ...
   TeamSTARS "tsWxGTUI_PyVx_SWIG" Toolkit may be released in
   ...
   3.1 "Git" Repository Clone "Zip" File
       ...
       "tsWxGTUI_PyVx_SWIG" Toolkit to your computer's desktop or
       ...
       obtain a copy of the toolkit repository:
          ...
          https://github.com/rigordo959/tsWxGTUI_PyVX_SWIG_Repository
   ...
   3.2 "Zip" File for Microsoft Windows
       ...
       sdist-stage source code and documentation products:
           ...
           "tsWxGTUI_PyVx_SWIG-3.2.1.zip"    (106,632,641 bytes)

       Single-generation Python programming language release
       sdist-stage source code products:

           "tsWxGTUI_Py3x_SWIG-3.2.1.zip"
           "tsWxGTUI_Py2x_SWIG-3.2.1.zip"    (  2,251,821 bytes)
           "tsWxGTUI_Py1x_SWIG-3.2.1.zip"
   ...
   3.3 "Tar" File for Cygwin, GNU/Linux and Unix
       ...
       sdist-stage source code and documentation products:
          ...
          "tsWxGTUI_PyVx_SWIG-3.2.1.tar"    (292,465,280 bytes)
          "tsWxGTUI_PyVx_SWIG-3.2.1.tar.gz" (104,901,314 bytes)
          "tsWxGTUI_PyVx_SWIG-3.2.1.tgz"    (104,901,314 bytes)

       Single-generation Python programming language release
       sdist-stage source code products:

          "tsWxGTUI_Py3x_SWIG-3.2.1.tar.gz"
          "tsWxGTUI_Py2x_SWIG-3.2.1.tar.gz" (  1,769,671 bytes)
          "tsWxGTUI_Py1x_SWIG-3.2.1.tar.gz"

       Single-generation Python programming language release
       build-stage products:

          "tsWxGTUI_Py2x_SWIG build"        ( 10,992,421 bytes)
          "tsWxGTUI_Py2x_SWIG.egg-info"     (     75,456 bytes)
       ...
       install-stage products:
          ...
          "tsWxGTUI_Py2x_SWIG dist"         (  4,036,623 bytes)
          "tsWxGTUI_Py2x_SWIG-0.0.0-py2.7.egg"
                                       ( 16,266,200 bytes)
...
5. Sdist, Build & Install Contents & Organization
   ...
   The TeamSTARS "tsWxGTUI_PyVx_SWIG" Toolkit has been engineered
   ...
   5.1 The command "python setup.py sdist" can be used to
       ...
       a) A "tarball" file named "tsWxGTUI_PyVx_SWIG-0.0.0.tar.gz"

          or

       b) A "zip" file named "tsWxGTUI_PyVx_SWIG-0.0.0.zip".

       The "tarball" and "zip" files contain a directory
       named ["tsWxGTUI_PyVx_SWIG-0.0.0"]. It contains the
       following:

       ["tsWxGTUI_PyVx_SWIG_Repository-0.0.0"]
         |
         +-- ["tsWxGTUI_PyVx_SWIG"]
         |     |
         |     +-- ["tsDemoArchive"]
       ...
       5.3.1 Cygwin (a free Linux-like Command Line Interface
             ...
             ["/cygwin/lib"]
               ...
               +-- ["python3.2"]
                     ...
                     +-- ["site-packages"]
                           |
                           +-- ["tsWxGTUI_Py3x_SWIG-0.0.0-py3.2.egg"]
                                 |
                                 +-- ["EGG-INFO"]
                                 |     |
                                 |     +-- ["scripts"]
