      PROGRAM F77TST
C
C        THIS BENCHMARK CONSISTS OF TEN MODULES, EACH OF WHICH
C        EXERCISES A GROUP OF LANGUAGE FEATURES.  EACH MODULE
C        IS PLACED IN A LOOP AND THE NUMBER OF TIMES IT IS
C        EXERCISED IS ADJUSTED TO MIMIC, AS CLOSELY AS POSSIBLE,
C        THE AVAILABLE STATISTICAL PROFILES OF LANGUAGE FEATURE
C        USAGE.  ALL THE LOOPS HAVE BEEN ARRANGED SO THAT AN
C        OPTIMIZING COMPILER CANNOT REMOVE A SIGNIFICANT AMOUNT
C        OF CODE FROM THEM.
C
C        FEATURES EXERCISED INCLUDE SIMPLE VARIABLE AND ARRAY
C        ADDRESSING, FIXED AND FLOATING POINT ARITHMETIC, SUB-
C        ROUTINE CALLS AND PARAMETER PASSING, AND STANDARD MATHE-
C        MATICAL FUNCTIONS.
C
C*    DOUBLE PRECISION X1, X2, X3, X4, X, Y, Z, T, T1, T2, E1
      COMMON T, T1, T2, E1(4), J, K, L
C
      COMMON /UNITS/ CI, CO
      INTEGER CI, CO
C
C**** CHANGE FOR SYSTEM COMPATIBILITY *****
C
      CI = 5
      CO = 6
C
C**** OPEN ( UNIT=CI, FILE=':CI:' )
C**** OPEN ( UNIT=CO, FILE=':CO:', CARRIAGE='FORTRAN' )
      OPEN ( UNIT=CI )
      OPEN ( UNIT=CO )
C
C******************************************
...
910   FORMAT ( I2 )
C
C**** CHANGE FOR MAXIMUM ITERATION COUNT *****
...
C
      ...
      IRUN=0
C
C**** START OF THE TIMING ROUTINE *****
      CALL TIMRB
C****                             *****
C AFTER EACH TIMED LOOP, MAIN PROGRAM COMES HERE
979   T =0.499975
      T1=0.50025
      T2=2.0
      IRUN=IRUN+1
...
C
      IF ( N7 ) 79, 79, 71
71    DO 78 I=1, N7, 1
C           X=T*DATAN(T2*DSIN(X)*DCOS(X)/(DCOS(X+Y)+DCOS(X-Y)-1.0D0))
            X=T*ATAN(T2*SIN(X)*COS(X)/(COS(X+Y)+COS(X-Y)-1.0))
C           Y=T*DATAN(T2*DSIN(Y)*DCOS(Y)/(DCOS(X+Y)+DCOS(X-Y)-1.0D0))
...
C118  X=DSQRT(DEXP(DLOG(X)/T1))
118   X=SQRT(EXP(ALOG(X)/T1))
119   CONTINUE
      CALL POUT( N11, J, K, X, X, X, X )
      DELTA=SECNDS( TIME )
C END MAIN TIMED LOOP
C**** END OF THE TIMING ROUTINE *****
      CALL TIMRE
C****                    *****
...
C
C*****************************************************
C**** DETERMINE TIME OF DAY AS MEASURED IN HOURS *****
C****     NOTE: MANUAL TIMEKEEPING HAS           *****
C****           BEEN PRESUMED FOR THIS           *****
C****           IMPLIMENTATION                   *****
C*****************************************************
C
      BCLK = 0.0
