#! /usr/bin/env python
#"Time-stamp: <09/20/2013  4:39:31 AM rsg>"
'''
test_tsWxMetrics.py - Application program to mimic some features of
the Graphical Text User Interface.
'''
#################################################################
#
# File: test_tsWxMetrics.py
#
# Purpose:
#
...
systemSettingsTestEnabled = True
...
__help__ = '''
This program demonstrates currently supported metrics.
...
##            theValue = self.settings.GetScreenType()
    ...
    def verboseCase(self):
        ...
        winFrame = wx.Frame(
            ...
            id=wx.ID_ANY,
            title='Get_Metric',
            pos=wx.Point(16 + (0 * 380), 232),
            size=wx.Size(- 16 + 380 + 240, 100),
            style=wx.DEFAULT_FRAME_STYLE,
            name=wx.FrameNameStr)
        ...
        try:
            winFrameText  = wx.TextCtrl(
                winFrame,
                id=-1,
                value=wx.EmptyString,
                pos=wx.Point(
                    winFrame.ts_Rect.x + wx.pixelWidthPerCharacter,
                    winFrame.ts_Rect.y + wx.pixelHeightPerCharacter),
                size=wx.Size(
                    winFrame.ts_Rect.width - 2 * wx.pixelWidthPerCharacter,
                    winFrame.ts_Rect.height - 2 * wx.pixelHeightPerCharacter),
       ...
       # -----
        ...
        for theKey in keyList:
            ...
            if (not (theValue is None)):
                winFrameText.AppendText(
                    '\t%3d %40s %s' % (theKey, theName, theValue))
            print('\t%3d %40s %s' % (theKey, theName, theValue))
...
if __name__ == '__main__':
    ...
    buildTitle     = 'test_tsWxMetrics'
    buildVersion   = '1.0.0'
    buildDate      = '10/31/2011'
    ...
    guiTopLevelObjectTitle = 'metrics_communicate'
